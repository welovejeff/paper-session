...
The **setup-card conventions** are a second coupling of the same kind, added with the no-printer path (`research/0002`): the card's `I PROPOSE` caption and quotation marks mark machine content in the human's handwriting, a struck-as-written category block is a Spent Ground fence rather than a set of kills, and corner numbers replace the printed footer numbering. `paper-session/references/prompt-craft.md` §10 writes those conventions and `scan-back/SKILL.md` Step 1 ("Unprinted pages") reads them back, again with no shared file. **A change to either must be made in both**, and note the cascade differs on an unprinted page — legend > defaults, since no printed key exists to sit between them.
